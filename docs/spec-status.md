...
| v0.9.0a1 | Removed `"semi_trusted"` from the conformant `provenance[].trust` enum. Deleted the v0.8.1 warning/normalization compatibility path (`PICSemiTrustedDeprecationWarning`, `TrustLevel.SEMI_TRUSTED`, and the model-validation normalization bridge). | Breaking for producers still emitting `provenance[].trust = "semi_trusted"`: such proposals fail JSON Schema validation with `PIC_SCHEMA_INVALID`. Producers using `"trusted"` or `"untrusted"` are unaffected. See [`migration-trust-sanitization.md`](migration-trust-sanitization.md) for the migration path. |
...
**Current Python reference implementation:** v0.8.3
