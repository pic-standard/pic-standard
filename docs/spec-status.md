...
| v0.8.3 | Tightened hash-evidence semantics. Hash verification establishes content integrity only and MUST NOT upgrade `provenance[].trust` by itself. Signature evidence remains authority-bearing through the configured keyring. Added conformance coverage for the hash-no-trust-upgrade falsifier from issue #133. | Wire format unchanged. Semantics tightened for verifiers that previously treated hash-only evidence as authority-bearing. |
| v0.9.0a1 | Alpha preparation for the v0.9.0 cross-implementation milestone. Removes the conformant `"semi_trusted"` trust value and its v0.8.1 warning/normalization compatibility path. Removes the deprecated `apply_verified_ids_to_provenance` wrapper. Adds the `docs/ERRORS.md` error reference and enum/documentation parity test. Resolves `OQ-CORE-001` with explicit error-code precedence, clarifies the `PIC_SCHEMA_INVALID` / `PIC_VERIFIER_FAILED` boundary, and pins lone-surrogate rejection through `canonical_reject`, `canon-010-lone-surrogate-rejection`, and a pipeline-level `PIC_SCHEMA_INVALID` guard. Adds the OpenAPI 3.0.3 bridge contract, Spectral CI, `/v1/version` machine-readable metadata, centralized invalid `X-Request-ID` rejection, and a hardened reference Docker Compose profile with deployment docs. | Protocol-level breaking change for producers still emitting `provenance[].trust = "semi_trusted"`: such proposals fail JSON Schema validation with `PIC_SCHEMA_INVALID`. Existing positive canonicalization vectors remain byte-identical; v0.9.0a1 adds the first negative canonicalization vector and `canonical_reject` verdict. HTTP/deployment surfaces also change: invalid `X-Request-ID` now returns HTTP 400 / `PIC_INVALID_REQUEST`, and the reference Compose profile binds to `127.0.0.1:7580` by default. See [`CHANGELOG.md`](../CHANGELOG.md) and [`migration-trust-sanitization.md`](migration-trust-sanitization.md). |
| v0.9.0a2 | Alpha 2 — Independent Review Hardening. Adds `PIC_DUPLICATE_ID` error code and pipeline step 2c uniqueness check (after JSON Schema validation and lone-surrogate rejection, before impact resolution, tool binding, evidence verification, and causal-taint evaluation). Tightens SHA-256 wire representation to lowercase hex only (`^[a-f0-9]{64}$`). Tightens Base64 signature representation (URL-safe, unpadded, overpadded, whitespace-containing, and wrong-decoded-length signatures rejected fail-closed). Removes `.strip()` whitespace normalization from exact tool binding. Flips `PipelineOptions.strict_trust` default from `False` to `True` (secure default) with `PICLegacyTrustModeWarning` at explicit `False` opt-in; adapters (LangGraph, MCP guard, HTTP bridge `handle_verify`) and CLI (`pic-cli verify --legacy-trust`) mirror the new default. Adds 11 new conformance vectors (73 → 84) covering spec-fix regressions and adapted-vector provenance for PIC-authored independent-review documentation. | Wire format unchanged. Under the new secure default, previously-passing proposals with self-asserted `trust="trusted"` will block for high-impact actions unless authority-bearing signature evidence is attached, or unless the caller explicitly opts into `strict_trust=False`. |
| v0.9.0 | Stable cross-implementation milestone. First TypeScript verifier pass at [`pic-standard/pic-standard-ts`](https://github.com/pic-standard/pic-standard-ts) covering `canonicalization`, `core`, and `trust_sanitization` modes on the shared `conformance/manifest.json`. Advisory cross-implementation differential CI in this repository via `scripts/diff_conformance.py` and `.github/workflows/differential.yml`. Adds `docs/canonicalization.md` §7.14 (host-language input validation, Normative) and `docs/spec-core.md` §12.1 (cross-implementation semantic subset, Normative). Refines `docs/spec-core.md` §7.1 tool-binding wording for absent-or-empty `expected_tool` precision. Evidence-mode TypeScript parity remains a v0.9.x completion item. | None. Additive: TypeScript parity, differential-CI harness, and normative additions codifying already-shipped runtime behavior. No proposal wire-format change. |
...
**Current Python reference implementation:** v0.9.0
